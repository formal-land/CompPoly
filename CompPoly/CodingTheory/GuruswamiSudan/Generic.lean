...
/-!
# Generic Executable Guruswami-Sudan Decoder
...
brute-force fallbacks. It is parameterized over the executable field
representation so canonical and native-word KoalaBear backends share one
implementation.
-/
...
variable {F : Type} [Field F] [BEq F] [Inhabited F]

namespace UniPoly

def zero : UniPoly F := #[0]

def one : UniPoly F := #[1]

def isZero (p : UniPoly F) : Bool :=
  p.all (fun c => c == 0)

def trim (p : UniPoly F) : UniPoly F := Id.run do
  let mut last := p.size
...
def coeff (p : UniPoly F) (i : Nat) : F :=
  p.getD i 0

def degree (p : UniPoly F) : Nat :=
  let q := trim p
  if q.isEmpty || isZero q then 0 else q.size - 1

def leadingCoeff (p : UniPoly F) : F :=
  let q := trim p
  q.getD (q.size - 1) 0

def ofNatArray (xs : Array Nat) : UniPoly F :=
  let ys : Array F := xs.map (fun x : Nat => (x : F))
  trim ys

def neg (p : UniPoly F) : UniPoly F :=
  trim (p.map fun c => -c)

def add (p q : UniPoly F) : UniPoly F := Id.run do
...
def sub (p q : UniPoly F) : UniPoly F :=
  add p (neg q)

def scale (a : F) (p : UniPoly F) : UniPoly F :=
  trim (p.map fun c => a * c)

def mul (p q : UniPoly F) : UniPoly F := Id.run do
...
def mulXPow (p : UniPoly F) (k : Nat) : UniPoly F :=
  if isZero p then
    zero
  else
    trim (Array.replicate k (0 : F) ++ p)

def evaluate (p : UniPoly F) (x : F) : F :=
  p.foldr (fun c acc => acc * x + c) 0

def pow (p : UniPoly F) (e : Nat) : UniPoly F := Id.run do
  let mut result := one
  for _ in [0:e] do
    result := mul result p
  pure result

def monomial (i : Nat) (c : F) : UniPoly F :=
  if c == 0 then
    zero
  else
    (Array.replicate i (0 : F)).push c

def longDivRem? (num den : UniPoly F) : Option (Prod (UniPoly F) (UniPoly F)) := Id.run do
  let den := trim den
  if isZero den then
    pure none
  else
...
def coeffsEq (p q : UniPoly F) : Bool :=
  trim p == trim q
...
def zero : Bivariate F := { coeffs := #[UniPoly.zero] }

def trimCoeffs (coeffs : Array (UniPoly F)) : Array (UniPoly F) := Id.run do
...
def weightedDegree (q : Bivariate F) (w : Nat) : Nat := Id.run do
  let mut maxDeg := 0
  for j in [0:q.coeffs.size] do
...
def evaluateYPolynomial (q : Bivariate F) (f : UniPoly F) : UniPoly F := Id.run do
...
def fromMonomials (monomials : Array (Prod Nat Nat)) (coefficients : Array F) : Bivariate F := Id.run do
  let maxJ := monomials.foldl (fun acc pair => max acc pair.2) 0
  let mut rows : Array (UniPoly F) := Array.replicate (maxJ + 1) (#[] : UniPoly F)
...
def withDomain (domain : Array F) (k : Nat) : ReedSolomonCode F :=
...
def withConsecutiveDomain (n k : Nat) : ReedSolomonCode F :=
  withDomain (consecutiveDomain n) k

def pow2Domain (logN : Nat) (omega : F) : Array F := Id.run do
  let n := 2 ^ logN
  let mut out := #[]
  let mut cur := (1 : F)
  for _ in [0:n] do
...
def withRootsOfUnityDomain (logN k : Nat) (omega : F) : ReedSolomonCode F :=
  withDomain (pow2Domain logN omega) k

def encodePolynomial (code : ReedSolomonCode F) (poly : UniPoly F) : Array F :=
  code.domain.map fun x => UniPoly.evaluate poly x

def encode (code : ReedSolomonCode F) (message : Array F) : Array F :=
...
def substituteAndDivide (q : Bivariate F) (c : F) : Bivariate F := Id.run do
  let yDeg := Bivariate.yDegree q
  let maxXDeg := Bivariate.maxXDegree q + yDeg
  let maxYDeg := yDeg
  let mut result : Array (UniPoly F) :=
...
def findRootsLinearY (q : Bivariate F) (maxDegree : Nat) : Array (UniPoly F) :=
...
def appendUniquePoly (roots : Array (UniPoly F)) (candidate : UniPoly F) : Array (UniPoly F) :=
...
partial def enumerateSmallPolys
    (q : Bivariate F) (maxDegree adjustedMaxCoeff totalCandidates idx : Nat)
    (roots : Array (UniPoly F)) : Array (UniPoly F) :=
...
def trySmallIntegerPolynomials
    (q : Bivariate F) (maxDegree maxCoeff : Nat) (roots : Array (UniPoly F)) : Array (UniPoly F) :=
...
def tryDirectRoots
    (q : Bivariate F) (maxDegree : Nat) (hintValues : Array F)
    (roots : Array (UniPoly F)) : Array (UniPoly F) := Id.run do
  let mut roots := roots
  if maxDegree <= 4 then
...
partial def rrSearchWithDomain
    (q : Bivariate F) (maxDegree : Nat) (currentCoeffs : Array F)
    (roots : Array (UniPoly F)) (hintValues domain : Array F) (depth : Nat) : Array (UniPoly F) :=
  if roots.size >= maxTotalRoots then
    roots
  else
    Id.run do
...
def agreement (received domain : Array F) (poly : UniPoly F) : Nat := Id.run do
  let mut count := 0
...
def gsListDecodeWithMultiplicity
    (code : ReedSolomonCode F) (received : Array F) (multiplicity : Nat) : DecodeResult F :=
  let n := code.n
  let k := code.k
  let m := multiplicity
  let constraintsPerPoint := m * (m + 1) / 2
...
def gsListDecode (code : ReedSolomonCode F) (received : Array F) : DecodeResult F :=
...
def polyToNats (toNat : F → Nat) (p : UniPoly F) : Array Nat :=
  (UniPoly.trim p).map toNat

def valuesToNats (toNat : F → Nat) (xs : Array F) : Array Nat :=
  xs.map toNat

def candidatesToNats (toNat : F → Nat) (xs : Array (UniPoly F)) : Array (Array Nat) :=
  xs.map (polyToNats toNat)
